...
async function searchProwlarr(query, scope) {
    ...
    const [res, spanishIndexers] = await Promise.all([
        fetchWithTimeout(url, { headers: { "X-Api-Key": apiKey } }),
        getSpanishIndexerNames(base.replace(/\/$/, ""), apiKey),
    ]);
    if (!res.ok) {
    ...
    return data
        .map((r) => {
            ...
            // Se guarda la referencia al .torrent siempre que haya una
            // (aunque ya haya magnet directo): al cachear en Real-Debrid se
            // prefiere subir el .torrent real en vez del magnet, más
            // fiable para trackers privados (ver resolveDownloadRef).
