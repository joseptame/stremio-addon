...
async function resolveMagnet(r, apiKey) {
    ...
    try {
        const res = await fetchWithTimeout(r.downloadUrl, { headers: { "X-Api-Key": apiKey } }, 6000);
...
async function searchProwlarr(query, scope) {
    ...
    // Limita cuántos resultados se intentan resolver: de sobra para un
    // buscador manual, y evita tardar demasiado con búsquedas muy genéricas.
    const candidates = data.slice(0, 40);

    const withMagnets = await mapWithConcurrency(candidates, 6, async (r) => ({
        title: r.title || "",
        indexer: r.indexer || "",
        size: r.size || null,
