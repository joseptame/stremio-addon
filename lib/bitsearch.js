...
const { searchMovies, searchTvShows } = require("./tmdb");
...
// bitsearch.eu no da carátulas — se busca el título "limpio" en TMDB.
// Se corta todo lo que hay a partir del año de estreno (el release de un
// torrent siempre lo lleva en algún punto: calidad, grupo, idioma...
// vienen después), y se limpian puntos/corchetes/paréntesis sueltos.
function cleanTitleForLookup(title) {
    if (!title) return "";
    const yearMatch = (title || "").match(/\(?(19|20)\d{2}\)?/);
    const base = yearMatch ? title.slice(0, yearMatch.index) : title;
    return base
        .replace(/[._]/g, " ")
        .replace(/\[[^\]]*\]/g, " ")
        .replace(/\([^)]*\)/g, " ")
...
// Títulos distintos que se buscan en TMDB por búsqueda de bitsearch — un
// tope para no disparar decenas de peticiones a TMDB si hay muchos
// títulos distintos entre los resultados.
const MAX_POSTER_LOOKUPS = 20;

// Busca en TMDB la carátula de cada título de `titles` y la añade a
// `posterByTitle` (Map ya existente, se reutiliza entre llamadas para no
// repetir búsquedas de un título ya resuelto).
async function lookupPosters(titles, scope, posterByTitle) {
    const tmdbSearch = scope === "tv" ? searchTvShows : searchMovies;
    const pending = titles.filter((t) => t && !posterByTitle.has(t)).slice(0, MAX_POSTER_LOOKUPS);
    await Promise.all(pending.map(async (title) => {
        try {
            const results = await tmdbSearch(title);
            posterByTitle.set(title, (results[0] && results[0].poster) || null);
        } catch {
            // TMDB no configurado o caído: se sigue sin carátula para ese título.
            posterByTitle.set(title, null);
        }
    }));
}
...
async function searchBitsearch(query, scope) {
    ...
    // Las carátulas de la página 1 se piden a TMDB EN PARALELO con las
    // páginas de bitsearch que falten (no una cosa detrás de la otra):
    // así el tiempo de ambas se solapa en vez de sumarse, importante para
    // no acercarse al límite de ejecución de la función serverless.
    const firstPosterLookup = lookupPosters(
        matches.map((m) => cleanTitleForLookup(m.title)),
        scope,
        posterByTitle
    );
    ...
    await firstPosterLookup;
    // Segunda ronda solo para los títulos nuevos que hayan traído las
    // páginas 2+ y que la primera ronda (lanzada antes de tenerlas) no
    // pudo cubrir.
    await lookupPosters(matches.map((m) => cleanTitleForLookup(m.title)), scope, posterByTitle);

    matches.forEach((m) => { m.poster = posterByTitle.get(cleanTitleForLookup(m.title)) || null; });
